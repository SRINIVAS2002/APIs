...
fetch(link).then(r=>r.json()).then(fetch=>{
    ...
    document.write(`<h1 style="background-color: orange; padding : 10px; margin :50px; text-align: center;"> GAME NAME => GAME INDEX </h1>`)
    for(i of fetch["game_indices"]){
        document.write(`<h1 style="background-color: lightgreen; padding : 10px; margin :50px; text-align: center;"> ${i.version.name} => ${i.game_index} </h1>`)
    }
